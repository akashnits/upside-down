...
Everything server-side runs in one Apps Script web app. `doPost` in `router.js` is the only entry point; the `action` field on the request body selects the operation:

| Action | Caller | What it does |
| --- | --- | --- |
| `analyze` | Extension | Scores the job against your resume (tailored copy if one exists, else base) and returns the analysis. |
...
- **`router.js`** — `doPost` entry point and action dispatch.
...
Authorize the OAuth scopes when prompted (Drive, Docs, Sheets, external requests). Copy the resulting `/exec` **Web App URL**.
